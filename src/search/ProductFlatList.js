...
class ProductFlatList extends React.Component{
 ...
 componentDidMount() {
  ...
  render(){
    let {loading,Product,page,hasMore,loadMoreProduct,networkStatus,viewMode,sortMode} = this.props;
    ...
    return (
            <View style={styles.container}>
              <AnimatedFlatList
                ...
                contentContainerStyle={{paddingTop:TOOLBAR_HEIGHT}}
              ...
              />
              {loading && networkStatus !==3?<ActivityIndicator size="large" animating={loading && networkStatus !==3} style={styles.loading}/>:null}
            </View>);
  }
}
...
query productQuery($page:Int!,$pageSize:Int!,$groups:[Int!],$search:String,$spec:[Int!],$brand:[Int!],$sortMode:Int){
    Products:searchProductByCriteria(page:$page,pageSize:$pageSize,groups:$groups,searchText:$search,spec:$spec,brand:$brand,sortMode:$sortMode){
        page
        pageSize
        hasMore
        totalRows
...
const query = graphql(PRODUCT_QUERY,{
    options({page,criteria:{search,spec,brand},sortMode}){
        return {
            variables:{
                page:1,
                pageSize:10,
    ...
    props({ownProps:{criteria:{search}},data:{networkStatus,loading,Products,fetchMore,refetch}}){
